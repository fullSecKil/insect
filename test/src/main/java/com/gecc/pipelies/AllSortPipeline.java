...
import com.gecc.pojo.AllSort;
import com.gecc.pojo.Category;
...
/*
        List<Category> categoryList = allSort.getMobile();
        categoryList.stream().map(Category::getCategorys).map(hrefBeans -> hrefBeans.iterator().next()).forEach(hrefBean -> {
            String url = hrefBean.getUrl() + "&delivery=1&page=1&JL=4_10_0&go=0";
